...
or environment variables (DO_CLIENT_ID and DO_API_KEY)'''
    ...
    def build_inventory(self):
        ...
        for droplet in self.data['droplets']:
            ...
            # Strong convention for naming
            # based on environment.type.id
            # eg. production.webserver.1
            if len(components) == 3:
                environment = components[0]
                droplet_type = "%s_%s" % (components[0], components[1])

                self.push(self.inventory, environment, dest)
                self.push(self.inventory, droplet_type, dest)
